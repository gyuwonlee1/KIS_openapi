...
웹앱은 관리자 비밀번호로 로그인한 뒤 GitHub API로 `portfolio.json`을 읽고 저장합니다. 저장 전에는 종목 마스터 데이터와 조건 규칙으로 검증합니다.

웹앱에서 지원하는 작업:

- 종목명 또는 티커로 국내/미국 종목 검색
- 검색 결과에서 실제 종목을 선택해 관심 종목 추가
- 종목 활성화/비활성화
- 가격 조건 추가/수정/삭제
- 이동평균선 조건 추가/수정/삭제
- 이동평균선은 20일선, 60일선, 240일선, 480일선 중 선택
- 모든 조건은 최초 알림 후 자동 완료 처리

종목 검색 데이터는 `web/data/symbols/`의 정적 JSON을 사용합니다. 갱신이 필요하면 아래 명령을 실행해 공식 마스터 파일 기준으로 다시 생성합니다.
